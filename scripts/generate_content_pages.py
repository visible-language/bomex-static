...
def _doc(title: str, body_html: str, scripts_html: str = "", *, asset_prefix: str, root_prefix: str) -> str:
    ...
    return (
        ...
        "  <!-- GENERATED FILE: generated by scripts/generate_content_pages.py -->\n"
        "  <header></header>\n"
        f"{body_block}\n"
        "  <footer></footer>\n"
        f"{scroll_button}"
...
def _content_page_script_tag(asset_prefix: str) -> str:
    p = html.escape(asset_prefix)
    return f'<script src="{p}js/content.js"></script>'


def _detail_index_sidebar_script() -> str:
    return (
        "<script>\n"
        "(function(){\n"
        "  document.addEventListener('DOMContentLoaded', function(){\n"
        ...
        "\n"
        "        var current = (window.location.pathname.split('/').pop() || '').split('?')[0].split('#')[0];\n"
...
def _person_detail_scripts(item_id: str, asset_prefix: str) -> str:
    ...
    lines: List[str] = [_content_page_script_tag(asset_prefix)]
    if has_widgets:
        p = html.escape(asset_prefix)
        lines += [
...
def _person_detail(item: Item, *, output_dir: Path, asset_prefix: str) -> str:
    ...
    return (
        "\n".join(
            [
                "<section class=\"content-detail-layout\" data-index-source=\"index.html\" data-index-title=\"People\">",
                "<aside class=\"content-index-sidebar\" aria-label=\"People index\"></aside>",
...
def _concept_or_echo_detail(kind: str, item: Item, *, asset_prefix: str) -> str:
    ...
    body = [
        f"<section class=\"content-detail-layout\" data-index-source=\"index.html\" data-index-title=\"{html.escape(kind.title())}\">",
        "<aside class=\"content-index-sidebar\" aria-label=\"Section index\"></aside>",
        "<div class=\"content-detail-main\">",
        hero.rstrip("\n"),
        "<section class=\"page-content\">",
    ...
    for it in echoes:
        ...
        _write_text(
            ...
            _doc(
                ...
                scripts_html=_content_page_script_tag("../"),
                asset_prefix="../",
                root_prefix="../",
            ),
        )
