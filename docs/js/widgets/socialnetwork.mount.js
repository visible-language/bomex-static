(function (global) {
  ...
  function getAssetBase() {
    ...
    var url = new URL(rootPrefix + 'widgets/Widgets/SocialNetwork', global.location.href).toString();
    return url.replace(/\/$/, '');
  }
  ...
  function applyScopedStyles(root, assetBase) {
    ...
    return Promise.all([colorsReq, widgetReq]).then(function (texts) {
      ...
      style.setAttribute('data-widget-style', 'social-network');
      var prefix = '.vl-social-root';
      var base = prefix + ' {' +
        'display:block;' +
        'position:relative;' +
        'width:100%;' +
  ...
  function mountSocialNetwork(container, options) {
    var opts = options || {};
    var assetBase = getAssetBase();

    var root = document.createElement('div');
    root.className = 'vl-social-root';
    root.innerHTML = '<div class="widget-loading">Loading Conversation Network...</div>';
    container.innerHTML = '';
    ...
    buildTemplate(root, assetBase)
      ...
      .then(function () {
        ...
        global.SocialNetworkWidgetAssetBase = assetBase;
        return loadScriptFresh(assetBase + '/main.js');
      })
      .catch(function (err) {
        root.innerHTML = '<p>Unable to load Conversation Network.</p>';
        console.error(err);
      });

    return {
      update: function () {
        if (global.SocialNetworkWidgetApi && typeof global.SocialNetworkWidgetApi.resize === 'function') {
          global.SocialNetworkWidgetApi.resize();
        }
      },
      destroy: function () {
        if (global.SocialNetworkWidgetApi && typeof global.SocialNetworkWidgetApi.destroy === 'function') {
          global.SocialNetworkWidgetApi.destroy();
        }
        container.innerHTML = '';
      }
    };
  ...
  registry['conversation-network'] = { mount: mountSocialNetwork };
  registry['conversation'] = { mount: mountSocialNetwork };
})(window);
